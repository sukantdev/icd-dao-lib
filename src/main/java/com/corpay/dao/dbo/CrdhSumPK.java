package com.corpay.dao.dbo;

import jakarta.persistence.Basic;
import jakarta.persistence.Column;
import jakarta.persistence.Embeddable;
import jakarta.persistence.Temporal;
import jakarta.persistence.TemporalType;
import java.io.Serializable;
import java.util.Date;

@Embeddable
public class CrdhSumPK implements Serializable {

    @Basic(optional = false)
    @Column(name = "PSTD_DT")
    @Temporal(TemporalType.DATE)
    private Date pstdDt;
    @Basic(optional = false)
    @Column(name = "CRD_NBR")
    private String crdNbr;
    @Basic(optional = false)
    @Column(name = "LVL_SEQ")
    private short lvlSeq;
    @Basic(optional = false)
    @Column(name = "ACCT_NBR")
    private String acctNbr;
    @Basic(optional = false)
    @Column(name = "CUST_ID")
    private String custId;
    @Basic(optional = false)
    @Column(name = "EMP_NBR")
    private String empNbr;
    @Basic(optional = false)
    @Column(name = "LVL_VAL")
    private String lvlVal;

    public CrdhSumPK() {
    }

    public CrdhSumPK(Date pstdDt, String crdNbr, short lvlSeq, String acctNbr, String custId, String empNbr, String lvlVal) {
        this.pstdDt = pstdDt;
        this.crdNbr = crdNbr;
        this.lvlSeq = lvlSeq;
        this.acctNbr = acctNbr;
        this.custId = custId;
        this.empNbr = empNbr;
        this.lvlVal = lvlVal;
    }

    public Date getPstdDt() {
        return pstdDt;
    }

    public void setPstdDt(Date pstdDt) {
        this.pstdDt = pstdDt;
    }

    public String getCrdNbr() {
        return crdNbr;
    }

    public void setCrdNbr(String crdNbr) {
        this.crdNbr = crdNbr;
    }

    public short getLvlSeq() {
        return lvlSeq;
    }

    public void setLvlSeq(short lvlSeq) {
        this.lvlSeq = lvlSeq;
    }

    public String getAcctNbr() {
        return acctNbr;
    }

    public void setAcctNbr(String acctNbr) {
        this.acctNbr = acctNbr;
    }

    public String getCustId() {
        return custId;
    }

    public void setCustId(String custId) {
        this.custId = custId;
    }

    public String getEmpNbr() {
        return empNbr;
    }

    public void setEmpNbr(String empNbr) {
        this.empNbr = empNbr;
    }

    public String getLvlVal() {
        return lvlVal;
    }

    public void setLvlVal(String lvlVal) {
        this.lvlVal = lvlVal;
    }

    @Override
    public int hashCode() {
        int hash = 0;
        hash += (pstdDt != null ? pstdDt.hashCode() : 0);
        hash += (crdNbr != null ? crdNbr.hashCode() : 0);
        hash += (int) lvlSeq;
        hash += (acctNbr != null ? acctNbr.hashCode() : 0);
        hash += (custId != null ? custId.hashCode() : 0);
        hash += (empNbr != null ? empNbr.hashCode() : 0);
        hash += (lvlVal != null ? lvlVal.hashCode() : 0);
        return hash;
    }

    @Override
    public boolean equals(Object object) {

        if (!(object instanceof CrdhSumPK)) {
            return false;
        }
        CrdhSumPK other = (CrdhSumPK) object;
        if ((this.pstdDt == null && other.pstdDt != null) || (this.pstdDt != null && !this.pstdDt.equals(other.pstdDt))) {
            return false;
        }
        if ((this.crdNbr == null && other.crdNbr != null) || (this.crdNbr != null && !this.crdNbr.equals(other.crdNbr))) {
            return false;
        }
        if (this.lvlSeq != other.lvlSeq) {
            return false;
        }
        if ((this.acctNbr == null && other.acctNbr != null) || (this.acctNbr != null && !this.acctNbr.equals(other.acctNbr))) {
            return false;
        }
        if ((this.custId == null && other.custId != null) || (this.custId != null && !this.custId.equals(other.custId))) {
            return false;
        }
        if ((this.empNbr == null && other.empNbr != null) || (this.empNbr != null && !this.empNbr.equals(other.empNbr))) {
            return false;
        }
        if ((this.lvlVal == null && other.lvlVal != null) || (this.lvlVal != null && !this.lvlVal.equals(other.lvlVal))) {
            return false;
        }
        return true;
    }

    @Override
    public String toString() {
        return "com.corpay.dao.dbo.CrdhSumPK[ pstdDt=" + pstdDt + ", crdNbr=" + crdNbr + ", lvlSeq=" + lvlSeq + ", acctNbr=" + acctNbr + ", custId=" + custId + ", empNbr=" + empNbr + ", lvlVal=" + lvlVal + " ]";
    }

}
